...
static void app_card_create(smarthub_ctx_t * c, lv_obj_t * parent, const char * text,
...
void smarthub_home(lv_obj_t * base_obj)
{
    ...
    lv_subject_add_observer_obj(&c->th, smarthub_theme_observer_image_src_cb, bg_img, &c->imgs[IMG_LIGHT_BG_HOME]);
    ...
    smarthub_top_margin_create(bg_cont,
                                       c->sz == &smarthub_sizes_all[SIZE_SM] ? c->sz->gap[9] : c->sz->gap[10], false, c);
    ...
    lv_obj_t * date_and_time = smarthub_simple_container_create(info_area, true, c->sz->gap[4],
                                                                        LV_FLEX_ALIGN_START);
    ...
    lv_obj_t * weather = smarthub_simple_container_create(info_area, false, 50, LV_FLEX_ALIGN_END);
    lv_obj_t * weather_left = smarthub_simple_container_create(weather, true, c->sz->gap[2], LV_FLEX_ALIGN_START);
    ...
    lv_obj_t * weather_left_bottom = smarthub_simple_container_create(weather_left, true, c->sz->gap[3],
                                                                              LV_FLEX_ALIGN_START);
    ...
    lv_obj_t * weather_right = smarthub_simple_container_create(weather, true, c->sz->gap[2], LV_FLEX_ALIGN_CENTER);
    ...
    smarthub_label_bind_temperature(weather_temperature_label, &c->api.subjects.temperature_outdoor, c);
    ...
    app_card_create(c, apps, "EV Charging", c->imgs[IMG_EV_CHARGING_APP_ICON], smarthub_app_ev_charging);
    app_card_create(c, apps, "Smart Home", c->imgs[IMG_SMART_HOME_APP_ICON], smarthub_app_smart_home);
    app_card_create(c, apps, "Smart Meter", c->imgs[IMG_SMART_METER_APP_ICON], smarthub_app_smart_meter);
    app_card_create(c, apps, "Thermostat", c->imgs[IMG_THERMOSTAT_APP_ICON], smarthub_app_thermostat);
    app_card_create(c, apps, "About", c->imgs[IMG_ABOUT_APP_ICON], smarthub_app_about);
    ...
    lv_obj_t * theme_selector = smarthub_simple_container_create(bottom_margin, false, c->sz->gap[4],
    ...
    if(((const smarthub_theme_t *) lv_subject_get_pointer(&c->th)) == &smarthub_theme_light)
    ...
    lv_subject_add_observer_obj(&c->th, smarthub_theme_observer_image_src_cb, light_theme_icon,
    ...
    if(((const smarthub_theme_t *) lv_subject_get_pointer(&c->th)) == &smarthub_theme_dark)
    ...
    lv_subject_add_observer_obj(&c->th, smarthub_theme_observer_image_src_cb, dark_theme_icon,
...
static void set_light_theme_event_cb(lv_event_t * e)
{
    ...
    if(th == &smarthub_theme_light) return;

    lv_subject_set_pointer(&c->th, (void *)&smarthub_theme_light);
}
...
static void set_dark_theme_event_cb(lv_event_t * e)
{
    ...
    const smarthub_theme_t * th = lv_subject_get_pointer(&c->th);
    if(th == &smarthub_theme_dark) return;

    lv_subject_set_pointer(&c->th, (void *)&smarthub_theme_dark);
}

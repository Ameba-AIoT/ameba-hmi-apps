...
static void exit_cb(smarthub_api_t * api);
...
void smarthub_api_example(const char * assets_path, const char * logo_path, const char * slides_path)
{
    smarthub_api_t * api = lv_demo_smarthub(assets_path, logo_path, slides_path, exit_cb);

    /* see smarthub.h for documentation of the available subjects */
...
{
    /* slowly increment the time */
    smarthub_api_t * api = lv_timer_get_user_data(t);
...
static void wifi_ssid_observer_cb(lv_observer_t * observer, lv_subject_t * subject)
{
    smarthub_api_t * api = lv_observer_get_user_data(observer);

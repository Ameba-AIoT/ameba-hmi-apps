/**
 * @file smarthub_app_smart_meter.c
 *
 */
...
static lv_obj_t * create_widget1_part(smarthub_ctx_t * c, lv_obj_t * parent, const char * title_val,
                                      const char * daily_val, const char * weekly_val);
static void widget1_anim_height_range(smarthub_ctx_t * c, int32_t * min, int32_t * max);
...
static void create_widget1(smarthub_ctx_t * c, lv_obj_t * widgets);
static lv_obj_t * widget23_chart_label(smarthub_ctx_t * c, lv_obj_t * parent, const char * text);
...
static void create_widget2(smarthub_ctx_t * c, lv_obj_t * widgets);
...
static lv_obj_t * create_widget3_stat(smarthub_ctx_t * c, lv_obj_t * parent, const char * title_val,
                                      const char * kwh_val, const char * detail_val);
static void create_widget3(smarthub_ctx_t * c, lv_obj_t * widgets);
...
void smarthub_app_smart_meter(lv_obj_t * base_obj)
{
    ...
    lv_subject_add_observer_obj(&c->th, smarthub_theme_observer_image_src_cb, bg_img,
    ...
    int32_t app_padding = c->sz == &smarthub_sizes_all[SIZE_SM] ? c->sz->gap[9] : c->sz->gap[10];
    lv_obj_set_style_pad_bottom(bg_cont, app_padding, 0);
    lv_obj_set_style_pad_hor(bg_cont, app_padding, 0);

    /* top margin */

    lv_obj_t * top_margin = smarthub_top_margin_create(bg_cont, 0, true, c);

    /* app info */

    lv_obj_t * app_info = smarthub_simple_container_create(bg_cont, true, c->sz->gap[4], LV_FLEX_ALIGN_START);
    lv_obj_align_to(app_info, top_margin, LV_ALIGN_OUT_BOTTOM_LEFT, 0, c->sz->gap[7]);

    lv_obj_t * back = smarthub_simple_container_create(app_info, false, c->sz->gap[2], LV_FLEX_ALIGN_CENTER);
    ...
    int32_t widget_gap_padding = c->sz == &smarthub_sizes_all[SIZE_SM] ? c->sz->gap[4] : c->sz->gap[7];
    lv_obj_t * widgets = smarthub_simple_container_create(bg_cont, false, widget_gap_padding, LV_FLEX_ALIGN_END);
                                      ...
                                      const char * daily_val, const char * weekly_val)
{
    ...
    lv_subject_add_observer_obj(&c->th, smarthub_theme_observer_obj_bg_image_src_cb, part,
    ...
    lv_obj_t * daily_box = smarthub_simple_container_create(box, true, c->sz->gap[2], LV_FLEX_ALIGN_START);
    ...
    lv_obj_t * daily_value_box = smarthub_simple_container_create(daily_box, false, c->sz->gap[1],
                                                                          LV_FLEX_ALIGN_END);
    ...
    lv_obj_t * weekly_box = smarthub_simple_container_create(box, true, c->sz->gap[2], LV_FLEX_ALIGN_START);
    ...
    lv_obj_t * weekly_value_box = smarthub_simple_container_create(weekly_box, false, c->sz->gap[1],
                                                                           LV_FLEX_ALIGN_END);
...
{
    lv_obj_t * box = arg;
    smarthub_ctx_t * c = lv_obj_get_user_data(box);
    ...
    const smarthub_theme_t * th = lv_subject_get_pointer(&c->th);
    if(th == &smarthub_theme_dark) {
        int32_t g = lv_map(val, 0, 1000, 0, 255);
        int32_t ginv = 255 - g;
        lv_obj_set_style_text_color(lv_obj_get_child(part1, 0), th == &smarthub_theme_light
                                    ? lv_color_make(g, g, g) : lv_color_make(ginv, ginv, ginv), 0);
        lv_obj_set_style_text_color(lv_obj_get_child(part2, 0), th == &smarthub_theme_light
...
static void widget1_clicked_cb(lv_event_t * e)
{
    smarthub_ctx_t * c = lv_event_get_user_data(e);
...
static void create_widget1(smarthub_ctx_t * c, lv_obj_t * widgets)
{
    ...
    const smarthub_theme_t * th = lv_subject_get_pointer(&c->th);
    lv_obj_set_style_text_color(part_title, th == &smarthub_theme_light ? lv_color_black() : lv_color_white(), 0);
...
static void widget2_chart_event_draw_task_cb(lv_event_t * e)
{
    ...
    if(base_draw_dsc->part == LV_PART_MAIN) {
        ...
        rect_draw_dsc.bg_color = lv_subject_get_pointer(&c->th) == &smarthub_theme_light ? lv_color_black() :
                                 lv_color_white();
        ...
        lv_area_t a;
        a.x1 = coords.x1 + col_pos.x - w / 2;
        a.x2 = coords.x1 + col_pos.x + w / 2;
        a.y1 = coords.y1;
        a.y2 = coords.y2;
...
static void create_widget2(smarthub_ctx_t * c, lv_obj_t * widgets)
{
    ...
    lv_subject_add_observer_obj(&c->th, smarthub_theme_observer_obj_bg_image_src_cb, widget,
                                &c->imgs[IMG_LIGHT_WIDGET1_BG]);
    ...
    lv_obj_t * usage_box = smarthub_simple_container_create(widget, true, c->sz->gap[2], LV_FLEX_ALIGN_START);
    ...
    lv_obj_t * amount_box = smarthub_simple_container_create(usage_box, false, c->sz->gap[1], LV_FLEX_ALIGN_END);
...
static lv_obj_t * create_widget3_stat(smarthub_ctx_t * c, lv_obj_t * parent, const char * title_val,
                                      const char * kwh_val, const char * detail_val)
{
    lv_obj_t * obj = smarthub_simple_container_create(parent, true, c->sz->gap[4], LV_FLEX_ALIGN_START);
    ...
    lv_obj_t * kwh_box = smarthub_simple_container_create(obj, false, c->sz->gap[1], LV_FLEX_ALIGN_END);
...
static void create_widget3(smarthub_ctx_t * c, lv_obj_t * widgets)
{
    ...
    lv_obj_t * stat_box = smarthub_simple_container_create(widget, false, c->sz->gap[5], LV_FLEX_ALIGN_CENTER);

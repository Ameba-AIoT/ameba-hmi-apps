...
static lv_obj_t * create_button(lv_obj_t * parent, const void * img_src, smarthub_ctx_t * c);
...
void smarthub_app_about(lv_obj_t * base_obj)
{
    ...
    lv_obj_t * slide_deck = smarthub_simple_container_create(slide_deck_cont, false, c->sz->gap[2],
                                                                     LV_FLEX_ALIGN_START);
    lv_obj_set_style_pad_left(slide_deck, lv_display_get_horizontal_resolution(NULL) * 2 / 5, 0);

    lv_obj_t * buttons = smarthub_simple_container_create(slides_cont, false, c->sz->gap[2], LV_FLEX_ALIGN_START);
...
static void play_pause_clicked_cb(lv_event_t * e)
{
    ...
    lv_timer_t * play_pause_timer = lv_event_get_user_data(e);
    smarthub_ctx_t * c = lv_obj_get_user_data(play_pause);
